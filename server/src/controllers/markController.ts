...
export const getStudentMarksByDiscipline = async (req: Request, res: Response) => {
  const { studentId, disciplineId } = req.params;
  try {
    const marks = await db.select().from(mark).where(and(
      eq(mark.studentId, Number(studentId)),
      eq(mark.disciplineId, Number(disciplineId)
      )));
